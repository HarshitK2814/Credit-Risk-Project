...
# --- MODIFIED IMPORT STYLE ---
from backend.services.data_fetcher import get_yahoo_finance_data, get_market_sentiment_data, get_news_data
...
app = FastAPI(title="CredTech AI API", description="API for the Explainable Credit Intelligence Platform.", version="1.0.0")

# --- Background Task for Retraining (CORRECTED) ---
def retrain_model_background(ticker: str):
    ...
    try:
        ...
        if not yf_data:
            logging.error(f"[BACKGROUND] Failed to fetch yfinance data for {ticker}. Aborting retrain."); return
        ...
        all_features = engineer_features(yf_data, market_sentiment, news_data or [])
...
async def get_credit_score(ticker: str, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        logging.error(f"Data fetching failed: {e}"); raise HTTPException(status_code=500, detail="Failed to fetch data from external sources.")
